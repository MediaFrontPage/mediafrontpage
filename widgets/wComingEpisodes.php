...
function widgetComingEpisodesHeader() {
  ...
  echo '<script type="text/javascript" src="js/jquery.mousewheel.min.js"></script>';
  echo '<script type="text/javascript" src="js/parse_uri.js"></script>';
      ...
      function addHighSlide() {
        ...
        for (i=0; i < allHTMLTags.length; i++) {
          if (allHTMLTags[i].className == 'bannerThumb') {
            ...
            var uri = parseUri(allHTMLTags[i].src);
            var auth = uri.userInfo;
            var src = allHTMLTags[i].src.replace(auth+'@','');
            allHTMLTags[i].parentNode.setAttribute('href', src);
